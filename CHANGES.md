...
- **Step 5 — SEO foundation. (LARGELY DONE)** The base was already strong (`SEOMeta` emits canonical, full Open Graph, Twitter cards, JSON-LD; Person/Org/WebSite/Article/Book/Breadcrumb schema helpers; `robots.txt`; DB-driven `sitemap.xml`). Article and Book pages already emit Article/Book JSON-LD with clean canonicals. Fixed the real gap: the `/writing` canonical included the full query string, so every `?pillar=&sub=&audience=&q=` combo became its own canonical. It now consolidates to the primary dimension (`?pillar=`, `?track=`, or `?series=`, else bare `/writing`), killing the duplicate-content risk. Added the five pillar listing pages + Study Guides to the sitemap. Remaining nicety: pillar-page H1s could carry more literal search terms.
