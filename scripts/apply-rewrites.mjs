...
/**
 * content/rewrites/retire.json: {"thin-duplicate": "stronger-essay", ...}.
 * Retires a weaker duplicate into an essay that already clears the bar and is
 * not being rewritten. The retired essay leaves the library and every listing,
 * and its address 301s to the stronger one, exactly as a merge does.
 */
export function loadRetirements(root = ROOT) {
  const file = path.join(root, DIR, "retire.json");
...
export function applyToLibrary(records, rewrites, today = new Date().toISOString(), retirements = {}) {
  const merged = {};
  const live = new Set(records.map((r) => r && r.slug));
  for (const [from, to] of Object.entries(retirements)) {
    if (from === to) throw new Error(`${from}: retired into itself`);
    if (retirements[to]) throw new Error(`${from} retires into ${to}, which is itself retired`);
    if (!live.has(to) && !rewrites.some((rw) => rw.slug === to)) throw new Error(`${from} retires into ${to}, which is not in the library`);
    merged[from] = to;
  }
...
function main() {
  ...
  const { records: next, index } = applyToLibrary(records, rewrites, undefined, loadRetirements());
  ...
  // The client's copy of the merge map: an in-app link to a merged essay never
  // reaches Vercel's redirect (the router handles it), so the essay page sends
  // the reader on itself (client/src/pages/ArticleDetail.tsx).
  writeFileSync(p("client/src/data/merged-essays.json"), JSON.stringify(index.merged, null, 1) + "\n");
