/**
 * build-alone-pdf.mjs — render the "Alone in a Crowded Church" manuscript
 * (content/books/alone-in-a-crowded-church.md) into the gated ebook PDF served
 * by /api/download. Run:  node scripts/build-alone-pdf.mjs
 *
...
const SRC = path.join(ROOT, "content/books/alone-in-a-crowded-church.md");
const OUT = path.join(ROOT, "api/_ebooks/alone-in-a-crowded-church.pdf");
...
const EPIGRAPH = "Let him who cannot be alone beware of community. Let him who is not in community beware of being alone.";
const EPIGRAPH_BY = "Dietrich Bonhoeffer, Life Together";
...
doc.fontSize(9.5).fillColor(MUSTARD).text("PASTORS CONNECTION NETWORK", { align: "center", characterSpacing: 1.5 });
