...
/** The pastor-trade essays moved to PCN; their URLs 301 there (vercel.json). */
function movedToPcn() {
  try {
    return new Set(JSON.parse(fs.readFileSync(path.join(ROOT, "content/pcn-moved.json"), "utf8")).slugs ?? []);
  } catch {
    return new Set();
  }
}
...
function essayItems(dbRows, staticRows) {
  ...
  for (const r of [...(dbRows ?? []), ...staticRows]) {
    if (!r.slug || bySlug.has(r.slug) || takenDown.has(r.slug) || hidden.has(r.slug) || moved.has(r.slug)) continue;
