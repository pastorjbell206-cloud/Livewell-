# LiveWell Voice Audit

Source corpus: `/home/user/Livewell-/scripts/.essays-corpus.json`

**802 essays scanned** — 146 need a human pass, 656 are mechanically clean. 63 hard forbidden hits, 223 judgment-call flags.

_Mechanical only. A clean score means no banned words — not that the prose carries Bell's voice. Read the top of this list first._

## Ranked — essays needing the most attention

| # | Essay | Forbidden | Review | per 1k words |
|--:|-------|----------:|-------:|-------------:|
| 1 | 5. Neglecting Their Own Spiritual Life | 6 | 1 | 0.9 |
| 2 | client/public/family-devotions-2.json | 2 | 4 | 0.2 |
| 3 | client/public/studyguides/economic-justice.json | 2 | 3 | 0.1 |
| 4 | 17. Mobilizing Your Church for World Missions Without a Big Budget | 2 | 1 | 0.6 |
| 5 | client/public/studyguides/christian-nationalism.json | 2 | 1 | 0.1 |
| 6 | client/public/leadership/articles/expository-preaching.json | 2 | 0 | 1.8 |
| 7 | client/public/leadership/articles/crisis-care.json | 2 | 0 | 1.7 |
| 8 | It Is Okay to Doubt: What to Do With Your Questions | 2 | 0 | 0.8 |
| 9 | client/public/leadership/surveys/spiritual-gifts.json | 2 | 0 | 0.8 |
| 10 | client/public/leadership/sermons/2-timothy.json | 2 | 0 | 0.6 |
| 11 | client/public/leadership/articles-index.json | 2 | 0 | 0.5 |
| 12 | client/public/studyguides/pastoral-health.json | 1 | 11 | 0.0 |
| 13 | client/public/theology/passage-notes.json | 1 | 11 | 0.0 |
| 14 | client/public/studyguides/church-and-empire.json | 1 | 2 | 0.0 |
| 15 | client/public/theology/church-history-figures.json | 1 | 1 | 0.3 |
| 16 | client/public/family-catechism.json | 1 | 1 | 0.1 |
| 17 | client/public/studyguides/money.json | 1 | 1 | 0.1 |
| 18 | client/public/leadership/articles/how-to-do-a-word-study.json | 1 | 0 | 1.2 |
| 19 | client/public/leadership/articles/expository-vs-topical.json | 1 | 0 | 1.1 |
| 20 | client/public/leadership/articles/bible-study-tools.json | 1 | 0 | 1.1 |
| 21 | client/public/leadership/workflows/revitalization-plan.json | 1 | 0 | 1.1 |
| 22 | client/public/leadership/articles/lasting-youth-ministry.json | 1 | 0 | 1.1 |
| 23 | client/public/leadership/articles/how-to-study-the-bible.json | 1 | 0 | 1.0 |
| 24 | client/public/leadership/articles/the-leaders-prayer-life.json | 1 | 0 | 1.0 |
| 25 | client/public/context/guides/genesis-one-in-its-world.json | 1 | 0 | 1.0 |
| 26 | client/public/leadership/articles/helping-a-doubting-believer.json | 1 | 0 | 1.0 |
| 27 | client/public/leadership/articles/recovering-hospitality.json | 1 | 0 | 1.0 |
| 28 | client/public/leadership/articles/preaching-with-or-without-notes.json | 1 | 0 | 1.0 |
| 29 | client/public/leadership/articles/praying-for-your-people.json | 1 | 0 | 1.0 |
| 30 | client/public/context/guides/the-cities-of-paul.json | 1 | 0 | 1.0 |
| 31 | client/public/leadership/articles/how-to-start-a-sermon.json | 1 | 0 | 1.0 |
| 32 | client/public/leadership/articles/how-to-write-a-sermon.json | 1 | 0 | 0.9 |
| 33 | client/public/leadership/articles/sermon-application.json | 1 | 0 | 0.9 |
| 34 | client/public/leadership/articles/bridging-text-to-today.json | 1 | 0 | 0.9 |
| 35 | client/public/leadership/articles/ministry-teams.json | 1 | 0 | 0.8 |
| 36 | client/public/leadership/articles/how-to-read-proverbs.json | 1 | 0 | 0.8 |
| 37 | client/public/leadership/articles/pastoral-succession.json | 1 | 0 | 0.7 |
| 38 | Consumerism Is a Spiritual Formation | 1 | 0 | 0.7 |
| 39 | Replanting: When a Church Has to Start Over | 1 | 0 | 0.6 |
| 40 | Why Churches Close, and What Comes Next | 1 | 0 | 0.6 |
| 41 | Why Gen Z Is Coming Back to Church | 1 | 0 | 0.6 |
| 42 | Charity Is Not Justice: The Difference That Changes Everything | 1 | 0 | 0.5 |
| 43 | client/public/nation/render.json | 1 | 0 | 0.3 |
| 44 | client/public/leadership/surveys/church-health.json | 1 | 0 | 0.3 |
| 45 | client/public/life/domains/friendship-against-isolation.json | 1 | 0 | 0.2 |
| 46 | client/public/theology/problem-of-evil.json | 1 | 0 | 0.2 |
| 47 | client/public/theology/anthropology.json | 1 | 0 | 0.2 |
| 48 | client/public/leadership/servant-leadership-handbook.json | 1 | 0 | 0.1 |
| 49 | foster-care-unfinished-mandate | 0 | 20 | 0.0 |
| 50 | client/public/leadership/whole-bible-sermons.json | 0 | 11 | 0.0 |
| 51 | client/public/leadership/sermons/genesis.json | 0 | 8 | 0.0 |
| 52 | client/public/disruption/topics/war-and-peace.json | 0 | 6 | 0.0 |
| 53 | beatitudes-actually-promising | 0 | 6 | 0.0 |
| 54 | church-mental-health-justice | 0 | 6 | 0.0 |
| 55 | justice-not-political | 0 | 5 | 0.0 |
| 56 | client/public/leadership/formation-guides.json | 0 | 4 | 0.0 |
| 57 | client/public/disruption/topics/prosperity-success.json | 0 | 4 | 0.0 |
| 58 | client/public/justice/timeline.json | 0 | 3 | 0.0 |
| 59 | client/public/leadership/sermons/ephesians.json | 0 | 3 | 0.0 |
| 60 | client/public/leadership/sermons/1-peter.json | 0 | 3 | 0.0 |
| 61 | client/public/justice/topics/race-and-image.json | 0 | 3 | 0.0 |
| 62 | client/public/studyguides/doubt.json | 0 | 2 | 0.0 |
| 63 | client/public/life/domains/grief-and-loss.json | 0 | 2 | 0.0 |
| 64 | client/public/family-seasonal.json | 0 | 2 | 0.0 |
| 65 | client/public/leadership/sermons/ruth.json | 0 | 2 | 0.0 |
| 66 | client/public/theology/biblical-theology-themes.json | 0 | 2 | 0.0 |
| 67 | client/public/theology/biblical-theology-storyline.json | 0 | 2 | 0.0 |
| 68 | 11-what-pastors-need-to-know-about-depression-and-why-its-not-a-faith-problem | 0 | 2 | 0.0 |
| 69 | client/public/disruption/topics/christian-nation-myth.json | 0 | 2 | 0.0 |
| 70 | 3-how-to-return-to-ministry-after-burning-out | 0 | 2 | 0.0 |
| 71 | sabbath-is-resistance | 0 | 2 | 0.0 |
| 72 | reaching-rich-more-comfortable | 0 | 2 | 0.0 |
| 73 | burnout-nobody-talks-about | 0 | 2 | 0.0 |
| 74 | return-ministry-after-burnout | 0 | 2 | 0.0 |
| 75 | church-political-brand-step-back | 0 | 2 | 0.0 |
| 76 | difference-tired-and-done | 0 | 2 | 0.0 |
| 77 | Leading Through Conflict on a Church Team | 0 | 2 | 0.0 |
| 78 | why-poverty-political | 0 | 2 | 0.0 |
| 79 | 1-the-slow-burn-how-ministry-exhaustion-sneaks-up-on-you-before-you-see-it-coming | 0 | 2 | 0.0 |
| 80 | consistent-pro-life-ethic | 0 | 2 | 0.0 |
| 81 | client/public/leadership/sermons/mark.json | 0 | 1 | 0.0 |
| 82 | client/public/theology/origins.json | 0 | 1 | 0.0 |
| 83 | client/public/leadership/illustrations.json | 0 | 1 | 0.0 |
| 84 | client/public/leadership/sermons/james.json | 0 | 1 | 0.0 |
| 85 | client/public/leadership/sermons/galatians.json | 0 | 1 | 0.0 |
| 86 | client/public/leadership/sermon-series.json | 0 | 1 | 0.0 |
| 87 | client/public/leadership/formation/burnout-and-the-theology-of-rest.json | 0 | 1 | 0.0 |
| 88 | client/public/creeds/documents/thirty-nine-articles-summary.json | 0 | 1 | 0.0 |
| 89 | client/public/theology/bible-books.json | 0 | 1 | 0.0 |
| 90 | What Christian Nationalism Is and What It Is Not | 0 | 1 | 0.0 |
| 91 | client/public/family-devotions.json | 0 | 1 | 0.0 |
| 92 | What the Bible Actually Says About Money | 0 | 1 | 0.0 |
| 93 | client/public/leadership/services/wedding.json | 0 | 1 | 0.0 |
| 94 | How to Read the Bible Without Making It Say What You Want | 0 | 1 | 0.0 |
| 95 | client/public/creeds/documents/augsburg-confession-summary.json | 0 | 1 | 0.0 |
| 96 | client/public/disruption/topics/sexuality-and-body.json | 0 | 1 | 0.0 |
| 97 | justice-not-political-theological | 0 | 1 | 0.0 |
| 98 | client/public/disruption/topics/church-and-empire.json | 0 | 1 | 0.0 |
| 99 | theology-of-protest | 0 | 1 | 0.0 |
| 100 | Hasn't the Church Done Terrible Things? | 0 | 1 | 0.0 |
| 101 | scripture-meditation | 0 | 1 | 0.0 |
| 102 | client/public/leadership/sermons/titus.json | 0 | 1 | 0.0 |
| 103 | widow-orphan-stranger | 0 | 1 | 0.0 |
| 104 | client/public/justice/witnesses.json | 0 | 1 | 0.0 |
| 105 | pastor-politically-divided-gospel-central | 0 | 1 | 0.0 |
| 106 | client/public/justice/topics/creation-care.json | 0 | 1 | 0.0 |
| 107 | client/public/justice/topics/the-poor.json | 0 | 1 | 0.0 |
| 108 | 14-how-to-preach-the-same-gospel-to-people-who-are-nothing-alike | 0 | 1 | 0.0 |
| 109 | 4-the-difference-between-being-tired-and-being-done | 0 | 1 | 0.0 |
| 110 | church-megaproject-widows-mite | 0 | 1 | 0.0 |
| 111 | prosperity-gospel-injustice | 0 | 1 | 0.0 |
| 112 | vulnerable-people-not-in-pews | 0 | 1 | 0.0 |
| 113 | okay-to-see-counselor | 0 | 1 | 0.0 |
| 114 | when-preaching-gets-stale | 0 | 1 | 0.0 |
| 115 | when-man-pulpit-falling-apart | 0 | 1 | 0.0 |
| 116 | client/public/justice/topics/the-prisoner.json | 0 | 1 | 0.0 |
| 117 | client/public/justice/topics/systemic-sin.json | 0 | 1 | 0.0 |
| 118 | nehemiah-rebuilding | 0 | 1 | 0.0 |
| 119 | mission-is-not-optional | 0 | 1 | 0.0 |
| 120 | 35-how-three-small-churches-co-planted-one-new-congregation | 0 | 1 | 0.0 |
| 121 | talk-global-missions-moves-congregation | 0 | 1 | 0.0 |
| 122 | theology-of-repair | 0 | 1 | 0.0 |
| 123 | kingdom-of-god-not-what-you-think | 0 | 1 | 0.0 |
| 124 | when-god-bless-america-replaces-thy-kingdom-come | 0 | 1 | 0.0 |
| 125 | the-monster-in-the-mirror | 0 | 1 | 0.0 |
| 126 | poor-not-ministry-category | 0 | 1 | 0.0 |
| 127 | Why Your Church Should Require a Sabbatical | 0 | 1 | 0.0 |
| 128 | client/public/disruption/topics/consumer-comfort.json | 0 | 1 | 0.0 |
| 129 | Why the Prophets Made Everyone Uncomfortable | 0 | 1 | 0.0 |
| 130 | post-roe-church-responsibility | 0 | 1 | 0.0 |
| 131 | mission-sending-culture-never-sent | 0 | 1 | 0.0 |
| 132 | finding-your-barnabas | 0 | 1 | 0.0 |
| 133 | Expository vs. Topical Preaching: Which and When | 0 | 1 | 0.0 |
| 134 | support-life-after-birth | 0 | 1 | 0.0 |
| 135 | client/public/disruption/topics/therapeutic-gospel.json | 0 | 1 | 0.0 |
| 136 | client/public/justice/topics/the-wage.json | 0 | 1 | 0.0 |
| 137 | Why the Bible Is One Story, Not Sixty-Six Books | 0 | 1 | 0.0 |
| 138 | Fighting Fair: Conflict Without Casualties | 0 | 1 | 0.0 |
| 139 | client/public/context/guides/the-slavery-texts.json | 0 | 1 | 0.0 |
| 140 | client/public/context/guides/money-texts-we-explain-away.json | 0 | 1 | 0.0 |
| 141 | client/public/leadership/articles/the-biblical-covenants.json | 0 | 1 | 0.0 |
| 142 | client/public/leadership/articles/historical-context.json | 0 | 1 | 0.0 |
| 143 | client/public/leadership/articles/resilience-in-ministry.json | 0 | 1 | 0.0 |
| 144 | client/public/leadership/articles/comparison-and-envy-in-ministry.json | 0 | 1 | 0.0 |
| 145 | client/public/leadership/articles/humility-in-leadership.json | 0 | 1 | 0.0 |
| 146 | The Unfinished Church: Calling, Vision, and the Future God Is Building | 0 | 1 | 0.0 |

## Detail (flagged essays only)

### 5. Neglecting Their Own Spiritual Life
`5-neglecting-their-own-spiritual-life`

- **FORBIDDEN** · `landscape` · 3× · forbidden-word
  - …and gradually less able to see the landscape clearly. The blind spots multiply.…
  - …e the same religious and political landscape as the communities they serve. Pro…
  - …. Understanding the ecclesiastical landscape in their region — what denominatio…
- **FORBIDDEN** · `navigate` · 2× · forbidden-word
  - …ractical counsel: no pastor should navigate a significant church crisis withou…
  - …ften Middle Eastern Christians who navigate the same religious and political l…
- **FORBIDDEN** · `leverage` · 1× · forbidden-word
  - …ly, not just a local congregation. Leverage What You Already Have Most churche…
- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …why he believes it. I respect the journey that brought him there. And he ext…

### client/public/family-devotions-2.json
`lib:client/public/family-devotions-2.json`

- **FORBIDDEN** · `showing up` · 2× · therapy-speak
  - …ake. Faithfulness means God keeps showing up. Every morning the sun rises, and…
  - …y morning. Thank you that you keep showing up for me. Great is your faithfulness…
- _review_ · `blessed` · 4× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ce I cannot make on my own. Amen. Blessed are the poor in spirit, for theirs…
  - …r theirs is the kingdom of heaven. Blessed are those who mourn, for they will…
  - …gs made right, those are the truly blessed. It sounds backward, and that is…

### client/public/studyguides/economic-justice.json
`lib:client/public/studyguides/economic-justice.json`

- **FORBIDDEN** · `do the work` · 2× · therapy-speak
  - …he most extravagant charity cannot do the work that only justice does, because ch…
  - …g the same. So before session one, do the work yourself. Find the place where you…
- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …r reading, and money is not safely blessed, which refutes the prosperity prea…
  - …r reading, and money is not safely blessed, which refutes the prosperity prea…
  - …ed and a side that wants its owner blessed, the study has stopped doing its w…

### 17. Mobilizing Your Church for World Missions Without a Big Budget
`17-mobilizing-your-church-for-world-missions-without-a-big-budget`

- **FORBIDDEN** · `showing up` · 2× · therapy-speak
  - …arly paid off, and your name keeps showing up in denominational conversations ab…
  - …left to give — but they will keep showing up anyway, running on fumes and perfo…
- _review_ · `authentic` · 1× · review-word
  - forbidden as filler; 'authentic community' is a hard ban (caught separately)
  - …h the same energy you once gave to authentic relationship. You stop asking God…

### client/public/studyguides/christian-nationalism.json
`lib:client/public/studyguides/christian-nationalism.json`

- **FORBIDDEN** · `do the work` · 2× · therapy-speak
  - …es being identical? Let the image do the work. You are not trying to prove the g…
  - …g the same. So before session one, do the work yourself. Find the place where you…
- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ly assume God favored this army or blessed that founding. The capacity, draw…

### client/public/leadership/articles/expository-preaching.json
`lib:client/public/leadership/articles/expository-preaching.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …put it there and preach that too. Do the work of finding the author's point befo…
- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …ook sets the agenda. You just keep showing up.…

### client/public/leadership/articles/crisis-care.json
`lib:client/public/leadership/articles/crisis-care.json`

- **FORBIDDEN** · `showing up` · 2× · therapy-speak
  - …, the accident, the diagnosis, and showing up to the worst day of someone's life…
  - …a pastor carries. You cannot keep showing up to the worst day of other people's…

### It Is Okay to Doubt: What to Do With Your Questions
`teen-it-is-okay-to-doubt`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …your whole life. Third, actually do the work on the question kind of doubt. If…
- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …r really a believer. Fourth, keep showing up while you sort it all out, even on…

### client/public/leadership/surveys/spiritual-gifts.json
`lib:client/public/leadership/surveys/spiritual-gifts.json`

- **FORBIDDEN** · `do the work` · 2× · therapy-speak
  - …p in to meet them. I would rather do the work behind the scenes than stand in fr…
  - …gized by tasks that free others to do the work they are called to. Those with th…

### client/public/leadership/sermons/2-timothy.json
`lib:client/public/leadership/sermons/2-timothy.json`

- **FORBIDDEN** · `do the work` · 2× · therapy-speak
  - …I feel the rebuke. It is easier to do the work than to give it away. Entrusting i…
  - …be sober-minded, endure suffering, do the work of an evangelist, fulfill your min…

### client/public/leadership/articles-index.json
`lib:client/public/leadership/articles-index.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …ip the saints for the work, not to do the work while the saints applaud. The min…
- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …, the accident, the diagnosis, and showing up to the worst day of someone's life…

### client/public/studyguides/pastoral-health.json
`lib:client/public/studyguides/pastoral-health.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …g the same. So before session one, do the work yourself. Where have you earned lo…
- _review_ · `blessed` · 7× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …God rested on the seventh day and blessed it, not because he was tired but t…
  - …rested on the seventh day, and he blessed the seventh day and made it holy,…
  - …part of how the world is made. He blessed the seventh day and made it holy.…
- _review_ · `journey` · 4× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …despair sets in. He goes a day's journey into the wilderness and sits under…
  - …el comes a second time, saying the journey is too great for you. God names th…
  - …arge. But he himself went a day's journey into the wilderness and came and s…

### client/public/theology/passage-notes.json
`lib:client/public/theology/passage-notes.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …e between God sending help and God showing up himself. glory, weight, the heavy…
- _review_ · `blessed` · 11× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …he world, not against it. They are blessed to stand in the gap for everyone e…
  - …explicit. Psalm 1:2 pictures the blessed person delighting in the law and m…
  - …that all peoples on earth will be blessed through him, sets the aim of the w…

### client/public/studyguides/church-and-empire.json
`lib:client/public/studyguides/church-and-empire.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …g the same. So before session one, do the work yourself. Find the ordinary loyalt…
- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …except from God. That sentence has blessed tyrants for two thousand years. Bu…
  - …ning authorities. The sentence has blessed tyrants for two thousand years, qu…

### client/public/theology/church-history-figures.json
`lib:client/public/theology/church-history-figures.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …than the clergy did. Forbidden to do the work at home, he fled to the continent,…
- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …animals in the arena. On that long journey he wrote letters to churches along…

### client/public/family-catechism.json
`lib:client/public/family-catechism.json`

- **FORBIDDEN** · `god's got this` · 1× · forbidden-phrase
  - …fferent thing entirely from saying God's got this. Sovereign Lord, we rest in knowi…
- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …and show what he is like. And God blessed them. And God said to them, Be fru…

### client/public/studyguides/money.json
`lib:client/public/studyguides/money.json`

- **FORBIDDEN** · `foster` · 1× · forbidden-word
  - …The Freedom of Simplicity, Richard Foster, for readers who want to think abo…
- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ceiving and releasing. It is more blessed to give than to receive. The freed…

### client/public/leadership/articles/how-to-do-a-word-study.json
`lib:client/public/leadership/articles/how-to-do-a-word-study.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …of the errors in the first place. Do the work. Use the tools. And let the senten…

### client/public/leadership/articles/expository-vs-topical.json
`lib:client/public/leadership/articles/expository-vs-topical.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …ly compromise for men who will not do the work. The other camp treats expository…

### client/public/leadership/articles/bible-study-tools.json
`lib:client/public/leadership/articles/bible-study-tools.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …. The dividing line is whether you do the work. I have known farmers with four bo…

### client/public/leadership/workflows/revitalization-plan.json
`lib:client/public/leadership/workflows/revitalization-plan.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …astors quietly give up while still showing up. What will guard your own soul, y…

### client/public/leadership/articles/lasting-youth-ministry.json
`lib:client/public/leadership/articles/lasting-youth-ministry.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …kward kid in eleventh grade, still showing up after the funny ones moved on. Sur…

### client/public/leadership/articles/how-to-study-the-bible.json
`lib:client/public/leadership/articles/how-to-study-the-bible.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …lways glow. Some mornings you will do the work and feel nothing, and you will be…

### client/public/leadership/articles/the-leaders-prayer-life.json
`lib:client/public/leadership/articles/the-leaders-prayer-life.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …ee years, though you may. Begin by showing up tomorrow morning, late and ashamed…

### client/public/context/guides/genesis-one-in-its-world.json
`lib:client/public/context/guides/genesis-one-in-its-world.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …om the blood of a condemned god to do the work the gods despised. In Genesis, hum…

### client/public/leadership/articles/helping-a-doubting-believer.json
`lib:client/public/leadership/articles/helping-a-doubting-believer.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …g people, over time. And then keep showing up for them. Text them on a Thursday.…

### client/public/leadership/articles/recovering-hospitality.json
`lib:client/public/leadership/articles/recovering-hospitality.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …ospels and watch where Jesus keeps showing up. At dinner. Eating with tax collec…

### client/public/leadership/articles/preaching-with-or-without-notes.json
`lib:client/public/leadership/articles/preaching-with-or-without-notes.json`

- **FORBIDDEN** · `lean into` · 1× · therapy-speak
  - …an read the room and move with it, lean into the moment that catches fire, slow…

### client/public/leadership/articles/praying-for-your-people.json
`lib:client/public/leadership/articles/praying-for-your-people.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …mewhere down deep, we think we can do the work without it. Prayerlessness is not…

### client/public/context/guides/the-cities-of-paul.json
`lib:client/public/context/guides/the-cities-of-paul.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …rdinary Tuesday. Four cities will do the work. Watch how the local situation sur…

### client/public/leadership/articles/how-to-start-a-sermon.json
`lib:client/public/leadership/articles/how-to-start-a-sermon.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …le. No commentary. Let the picture do the work and trust them to feel it. The ho…

### client/public/leadership/articles/how-to-write-a-sermon.json
`lib:client/public/leadership/articles/how-to-write-a-sermon.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …-clearing. Get to the passage. And do the work of moving from what the text says…

### client/public/leadership/articles/sermon-application.json
`lib:client/public/leadership/articles/sermon-application.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …it is meant for someone else. So do the work of imagining the actual people. No…

### client/public/leadership/articles/bridging-text-to-today.json
`lib:client/public/leadership/articles/bridging-text-to-today.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …ou have a meaning to carry. So you do the work first. You read the passage in its…

### client/public/leadership/articles/ministry-teams.json
`lib:client/public/leadership/articles/ministry-teams.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …ip the saints for the work, not to do the work while the saints applaud. The One…

### client/public/leadership/articles/how-to-read-proverbs.json
`lib:client/public/leadership/articles/how-to-read-proverbs.json`

- **FORBIDDEN** · `lean into` · 1× · therapy-speak
  - …n or daughter learning to live can lean into the grain of reality rather than a…

### client/public/leadership/articles/pastoral-succession.json
`lib:client/public/leadership/articles/pastoral-succession.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …isis forcing your hand and you can do the work calmly and well. The pastor who wa…

### Consumerism Is a Spiritual Formation
`consumerism-is-a-spiritual-formation`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …repetition, only by years, only by showing up to the table on the mornings you w…

### Replanting: When a Church Has to Start Over
`replanting-when-a-church-starts-over`

- **FORBIDDEN** · `gospel-centered` · 1× · forbidden-phrase
  - …12. Center Church: Doing Balanced, Gospel-Centered Ministry in Your City. Grand Rapid…

### Why Churches Close, and What Comes Next
`why-churches-close-and-what-comes-next`

- **FORBIDDEN** · `landscape` · 1× · forbidden-word
  - …w Research Center. 2024. Religious Landscape Study. Washington: Pew Research Ce…

### Why Gen Z Is Coming Back to Church
`why-gen-z-is-coming-back-to-church`

- **FORBIDDEN** · `landscape` · 1× · forbidden-word
  - …w Research Center. 2024. Religious Landscape Study. Washington: Pew Research Ce…

### Charity Is Not Justice: The Difference That Changes Everything
`charity-is-not-justice-the-difference`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …he most extravagant charity cannot do the work that only justice does, because ch…

### client/public/nation/render.json
`lib:client/public/nation/render.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …ubles cost you something. It means showing up for the long, local, repetitive wo…

### client/public/leadership/surveys/church-health.json
`lib:client/public/leadership/surveys/church-health.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …If someone in our church stopped showing up, people who know them would notice…

### client/public/life/domains/friendship-against-isolation.json
`lib:client/public/life/domains/friendship-against-isolation.json`

- **FORBIDDEN** · `showing up` · 1× · therapy-speak
  - …w realization that if they stopped showing up, the welcome team would miss them…

### client/public/theology/problem-of-evil.json
`lib:client/public/theology/problem-of-evil.json`

- **FORBIDDEN** · `god's got this` · 1× · forbidden-phrase
  - …that meets a grieving mother with God's got this, or everything happens for a reaso…

### client/public/theology/anthropology.json
`lib:client/public/theology/anthropology.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …a creature with the capacities can do the work and live in the relationship. Whe…

### client/public/leadership/servant-leadership-handbook.json
`lib:client/public/leadership/servant-leadership-handbook.json`

- **FORBIDDEN** · `do the work` · 1× · therapy-speak
  - …the assumption that a man who can do the work has therefore become the kind of m…

### foster-care-unfinished-mandate
`foster-care-unfinished-mandate`

- _review_ · `foster` · 20× · review-word
  - literal 'foster care/parent/etc.' — almost certainly fine; flagged only because 'foster' is on the filler list
  - …y 400,000 children in the American foster care system on any given day [cite…
  - …e 400,000 children in the American foster system as a different category fro…
  - …it. The church's engagement with foster care has been inconsistent and, in…

### client/public/leadership/whole-bible-sermons.json
`lib:client/public/leadership/whole-bible-sermons.json`

- _review_ · `blessed` · 10× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …des the heavens to their help. He blessed them on his way out. Moses climbs…
  - …st has already done in the first. Blessed With Every Blessing The Father ha…
  - …ly to the one who asks, and counts blessed not the hearer of the word but the…
- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …people. Strength Made Perfect: A Journey Through 2 Corinthians Paul writes…

### client/public/leadership/sermons/genesis.json
`lib:client/public/leadership/sermons/genesis.json`

- _review_ · `blessed` · 8× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth shall be blessed" (Genesis 12:2–3). Stand back and…
  - …of his own. Everything is gift. Blessed in order to bless Notice the sh…
  - …ch forgets it constantly. Abram is blessed, but the blessing does not termina…

### client/public/disruption/topics/war-and-peace.json
`lib:client/public/disruption/topics/war-and-peace.json`

- _review_ · `blessed` · 6× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …he Gospel. Matthew 5:9, 38 to 44 (blessed are the peacemakers, turn the othe…
  - …ame Lord. And the church has also blessed what it should have buried. It pre…
  - …ive with the knowledge that he has blessed killing and may have blessed it wr…

### beatitudes-actually-promising
`beatitudes-actually-promising`

- _review_ · `blessed` · 6× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …out allowing them to be a claim. "Blessed are the poor in spirit, for theirs…
  - …s version is less easy to manage. "Blessed are the poor" — without the qualif…
  - …Chose The Greek word translated "blessed" is makarios — a word that in it…

### church-mental-health-justice
`church-mental-health-justice`

- _review_ · `journey` · 6× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …Not for a few days. He ran a day's journey into the wilderness, sat under a t…
  - …the provision. Then the word: "The journey is too great for you" (1 Kings 19:…
  - …and honest acknowledgment that the journey was too great for him. Then, event…

### justice-not-political
`justice-not-political`

- _review_ · `blessed` · 5× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …e one does not predict the other. "Blessed are the poor in spirit, for theirs…
  - …kingdom of heaven" (Matthew 5:3). "Blessed are those who hunger and thirst fo…
  - …n — those are the ones Jesus calls blessed. In Luke's version of the Beatitu…

### client/public/leadership/formation-guides.json
`lib:client/public/leadership/formation-guides.json`

- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …rd, long after the feeling fades. Blessed are you if you do them Pastors ra…
  - …ated for his faith — that man is blessed, and the Sermon on the Mount says…
  - …ions. Ask Jesus. The man who said "Blessed are the peacemakers" was nailed be…
- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …or safekeeping when you leave on a journey you might not survive. Paul is dyi…

### client/public/disruption/topics/prosperity-success.json
`lib:client/public/disruption/topics/prosperity-success.json`

- _review_ · `blessed` · 4× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …culture that already calls winning blessed. Jesus did not recruit the way we…
  - …familiar and more frank. Bigger is blessed, growth is favor, and the full par…
  - …ad traveled. Kate Bowler's history Blessed traces this as a distinctly Americ…

### client/public/justice/timeline.json
`lib:client/public/justice/timeline.json`

- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the point of a Frankish spear was blessed by churchmen who should have known…
  - …ead. The Conquest of the Americas Blessed by the Church As Spain and Portug…
  - …ed the conquistadors and too often blessed the violence, the forced labor of…

### client/public/leadership/sermons/ephesians.json
`lib:client/public/leadership/sermons/ephesians.json`

- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …The blessing is already given "Blessed be the God and Father of our Lord…
  - …of our Lord Jesus Christ, who has blessed us in Christ with every spiritual…
  - …phesians 1:3). Read it slowly. Has blessed. Past tense. Every blessing. Not m…

### client/public/leadership/sermons/1-peter.json
`lib:client/public/leadership/sermons/1-peter.json`

- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ach Listen to where he starts. "Blessed be the God and Father of our Lord…
  - …— the quiet ache when a brother is blessed. Slander — the thing you say when…
  - …r righteousness' sake, you will be blessed" (1 Peter 3:14). Not if you suffer…

### client/public/justice/topics/race-and-image.json
`lib:client/public/justice/topics/race-and-image.json`

- _review_ · `blessed` · 3× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …chapter and verse. Segregation was blessed by white congregations who saw no…
  - …n, white churches across the South blessed Jim Crow and the doctrine of separ…
  - …l and Dabney Jim Crow segregation blessed by white churches white church si…

### client/public/studyguides/doubt.json
`lib:client/public/studyguides/doubt.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …looked is the one who worshiped. Blessed are those who have not seen and ye…
  - …ll John what you hear and see, and blessed is the one who is not offended by…

### client/public/life/domains/grief-and-loss.json
`lib:client/public/life/domains/grief-and-loss.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ability. It is poverty. Jesus said blessed are those who mourn, which means m…
  - …urning is not a malfunction of the blessed life but one of its signatures in…

### client/public/family-seasonal.json
`lib:client/public/family-seasonal.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth would be blessed. Abram was old and had no childre…
  - …uting Hosanna to the Son of David, blessed is he who comes in the name of the…

### client/public/leadership/sermons/ruth.json
`lib:client/public/leadership/sermons/ruth.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …es for the first time in the book. Blessed be he of the LORD, who hath not le…
  - …gather around her and bless God. "Blessed be the LORD, which hath not left t…

### client/public/theology/biblical-theology-themes.json
`lib:client/public/theology/biblical-theology-themes.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth will be blessed. He gives Abraham a sign in his ow…
  - …his offspring all nations will be blessed. It passes the test when God provi…

### client/public/theology/biblical-theology-storyline.json
`lib:client/public/theology/biblical-theology-storyline.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth will be blessed. Abraham has no children and his w…
  - …promise that all nations would be blessed through Abraham's family finds its…

### 11-what-pastors-need-to-know-about-depression-and-why-its-not-a-faith-problem
`11-what-pastors-need-to-know-about-depression-and-why-its-not-a-faith-problem`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …and eat," the text says, "for the journey is too great for you" (1 Kings 19:…
  - …say: I see you. I am staying. The journey is too great to take alone. Let us…

### client/public/disruption/topics/christian-nation-myth.json
`lib:client/public/disruption/topics/christian-nation-myth.json`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …It happened. But the church also blessed what it should have damned. It bap…
  - …ed kings who burned dissenters and blessed armies marching under its own bann…

### 3-how-to-return-to-ministry-after-burning-out
`3-how-to-return-to-ministry-after-burning-out`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …ood, water, and sleep. Twice. "The journey is too great for you" (1 Kings 19:…
  - …u find your way. Take the time the journey actually requires.…

### sabbath-is-resistance
`sabbath-is-resistance`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …he seventh day. Therefore the LORD blessed the Sabbath day and made it holy"…
  - …bath, the day God had finished and blessed and set apart before the humans ha…

### reaching-rich-more-comfortable
`reaching-rich-more-comfortable`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …explicit than Matthew's version: "Blessed are you who are poor, for yours is…
  - …e lame, the blind, and you will be blessed" (Luke 14:12-14). He did not say…

### burnout-nobody-talks-about
`burnout-nobody-talks-about`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …ater and sleep. Twice. Then a long journey on the strength of that food — for…
  - …ng for him at the other end of the journey. God already knew. The naming was…

### return-ministry-after-burnout
`return-ministry-after-burnout`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …struction: "Arise and eat, for the journey is too great for you." That is it.…
  - …oo great for you." That is it. The journey is too great for you. Eat and slee…

### church-political-brand-step-back
`church-political-brand-step-back`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …unters the Beatitudes seriously — "Blessed are the peacemakers," "Blessed are…
  - …— "Blessed are the peacemakers," "Blessed are those who hunger and thirst fo…

### difference-tired-and-done
`difference-tired-and-done`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …and says: Arise and eat, for the journey is too great for you (1 Kings 19:…
  - …ly help you. Answer honestly. The journey is too great to travel by the maps…

### Leading Through Conflict on a Church Team
`leading-through-conflict-on-a-church-team`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …us and he is not wrong about much. Blessed are the peacemakers. A quarrelsome…
  - …mail. Jesus did not bless that. He blessed the harder and braver thing, the o…

### why-poverty-political
`why-poverty-political`

- _review_ · `blessed` · 2× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …6:20–26 is the starkest passage: "Blessed are you who are poor, for yours is…
  - …, for yours is the kingdom of God. Blessed are you who are hungry now, for yo…

### 1-the-slow-burn-how-ministry-exhaustion-sneaks-up-on-you-before-you-see-it-coming
`1-the-slow-burn-how-ministry-exhaustion-sneaks-up-on-you-before-you-see-it-coming`

- _review_ · `journey` · 2× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …omes back. "Arise and eat, for the journey is too great for you." The God of…
  - …ophet and says: arise and eat. The journey is too great for you. It is too…

### consistent-pro-life-ethic
`consistent-pro-life-ethic`

- _review_ · `foster` · 2× · review-word
  - literal 'foster care/parent/etc.' — almost certainly fine; flagged only because 'foster' is on the filler list
  - …would ask hard questions about the foster care system and the hundreds of th…
  - …ion. It looks like an adoption and foster care ministry that is as organized…

### client/public/leadership/sermons/mark.json
`lib:client/public/leadership/sermons/mark.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …f he is the Christ, the Son of the Blessed, he answers plainly, "I am," and t…

### client/public/theology/origins.json
`lib:client/public/theology/origins.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …he seventh day, therefore the Lord blessed the Sabbath. God grounds Israel's…

### client/public/leadership/illustrations.json
`lib:client/public/leadership/illustrations.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …and would not let go until he was blessed. He got his blessing, but he also…

### client/public/leadership/sermons/james.json
`lib:client/public/leadership/sermons/james.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …no shelter from the cold you just blessed them out of. James asks the only h…

### client/public/leadership/sermons/galatians.json
`lib:client/public/leadership/sermons/galatians.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …, that in him all nations would be blessed, was always aimed at the Gentiles…

### client/public/leadership/sermon-series.json
`lib:client/public/leadership/sermon-series.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …fore a Father who sees in secret. Blessed Are the Poor in Spirit The kingdo…

### client/public/leadership/formation/burnout-and-the-theology-of-rest.json
`lib:client/public/leadership/formation/burnout-and-the-theology-of-rest.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …finished his work and rested, and blessed the day and made it holy, Genesis…

### client/public/creeds/documents/thirty-nine-articles-summary.json
`lib:client/public/creeds/documents/thirty-nine-articles-summary.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ok man's nature in the womb of the blessed Virgin, so that two whole and perf…

### client/public/theology/bible-books.json
`lib:client/public/theology/bible-books.json`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …e Christ. Sermons that retell the journey, restate the covenant, and set lif…

### What Christian Nationalism Is and What It Is Not
`what-christian-nationalism-is-and-is-not`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …know that God favored this army or blessed that founding, when Scripture is f…

### client/public/family-devotions.json
`lib:client/public/family-devotions.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …e lame, the blind, and you will be blessed. Although they cannot repay you, y…

### What the Bible Actually Says About Money
`what-the-bible-actually-says-about-money-and-wealth`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …reduction, and money is not safely blessed, which refutes the prosperity prea…

### client/public/leadership/services/wedding.json
`lib:client/public/leadership/services/wedding.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …begin a covenant, so send them out blessed and let the weight of it follow th…

### How to Read the Bible Without Making It Say What You Want
`how-to-read-the-bible-without-making-it-say-what-you-want`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …One closes it certain that God has blessed the social order exactly as it sta…

### client/public/creeds/documents/augsburg-confession-summary.json
`lib:client/public/creeds/documents/augsburg-confession-summary.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ok human nature in the womb of the blessed Virgin Mary, so that there are two…

### client/public/disruption/topics/sexuality-and-body.json
`lib:client/public/disruption/topics/sexuality-and-body.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …t committed same-sex unions can be blessed. This is not a gap between believe…

### justice-not-political-theological
`justice-not-political-theological`

- _review_ · `authentic` · 1× · review-word
  - forbidden as filler; 'authentic community' is a hard ban (caught separately)
  - …). Religion without justice is not authentic religion with a deficiency. It is…

### client/public/disruption/topics/church-and-empire.json
`lib:client/public/disruption/topics/church-and-empire.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …oral center while the white church blessed the order around it. The Salvadora…

### theology-of-protest
`theology-of-protest`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …waved palms and shouted "Hosanna! Blessed is he who comes in the name of the…

### Hasn't the Church Done Terrible Things?
`apologetics-hasnt-the-church-done-terrible-things`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …art of the offense. The church has blessed slave ships. It has burned dissent…

### scripture-meditation
`scripture-meditation`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ceived. Psalm 1 begins there: the blessed person's delight is in the law o…

### client/public/leadership/sermons/titus.json
`lib:client/public/leadership/sermons/titus.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …aches us to wait. "Waiting for our blessed hope, the appearing of the glory o…

### widow-orphan-stranger
`widow-orphan-stranger`

- _review_ · `foster` · 1× · review-word
  - literal 'foster care/parent/etc.' — almost certainly fine; flagged only because 'foster' is on the filler list
  - …'s wage is the widow. The child in foster care is the orphan. The undocument…

### client/public/justice/witnesses.json
`lib:client/public/justice/witnesses.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …onfronting white churches that had blessed segregation. He has said himself…

### pastor-politically-divided-gospel-central
`pastor-politically-divided-gospel-central`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …nds party. They will know that the blessed are the poor in spirit, not the cu…

### client/public/justice/topics/creation-care.json
`lib:client/public/justice/topics/creation-care.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …itation it should have rebuked. It blessed the felling of forests and the fou…

### client/public/justice/topics/the-poor.json
`lib:client/public/justice/topics/the-poor.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …nobody can name. The same church blessed the slave ship and read sermons ov…

### 14-how-to-preach-the-same-gospel-to-people-who-are-nothing-alike
`14-how-to-preach-the-same-gospel-to-people-who-are-nothing-alike`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …with doctrine but with condition: "Blessed are the poor in spirit" (Matthew 5…

### 4-the-difference-between-being-tired-and-being-done
`4-the-difference-between-being-tired-and-being-done`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …ep — twice — and then a word: "The journey is too great for you." Not a verdi…

### church-megaproject-widows-mite
`church-megaproject-widows-mite`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the congregation was obedient, God blessed it, the building stands as a sign…

### prosperity-gospel-injustice
`prosperity-gospel-injustice`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …hatic about the obligations of the blessed toward the poor. Deuteronomy 15, t…

### vulnerable-people-not-in-pews
`vulnerable-people-not-in-pews`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …e church is. They need to make the journey to our building, to enter our spac…

### okay-to-see-counselor
`okay-to-see-counselor`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …companion for the next leg of the journey (1 Kings 19:4-21). God understood…

### when-preaching-gets-stale
`when-preaching-gets-stale`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …, water, and someone who said the journey is too great for you (1 Kings 19:…

### when-man-pulpit-falling-apart
`when-man-pulpit-falling-apart`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …ther," but "arise and eat, for the journey is too great for you" (1 Kings 19:…

### client/public/justice/topics/the-prisoner.json
`lib:client/public/justice/topics/the-prisoner.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …rough it. And the church has also blessed the gallows, baptized cruelty, and…

### client/public/justice/topics/systemic-sin.json
`lib:client/public/justice/topics/systemic-sin.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …aithful. And the church built and blessed those same structures more often t…

### nehemiah-rebuilding
`nehemiah-rebuilding`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …ashamed — and Nehemiah, five days' journey away in the Persian capital of Sus…

### mission-is-not-optional
`mission-is-not-optional`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ing: "all peoples on earth will be blessed through you" (Genesis 12:3). The o…

### 35-how-three-small-churches-co-planted-one-new-congregation
`35-how-three-small-churches-co-planted-one-new-congregation`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …Barnabas on their first missionary journey (Acts 13:1-3), was not the largest…

### talk-global-missions-moves-congregation
`talk-global-missions-moves-congregation`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the nations of the earth would be blessed through his offspring (Genesis 12:…

### theology-of-repair
`theology-of-repair`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ies — including many churches that blessed the white flight and the segregate…

### kingdom-of-god-not-what-you-think
`kingdom-of-god-not-what-you-think`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …he reality Jesus has inaugurated. "Blessed are the poor in spirit, for theirs…

### when-god-bless-america-replaces-thy-kingdom-come
`when-god-bless-america-replaces-thy-kingdom-come`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …tion, needs the Kingdom. Not to be blessed by it in the sense of preserved an…

### the-monster-in-the-mirror
`the-monster-in-the-mirror`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the Mount's reversals of status — "blessed are the poor" — with different eye…

### poor-not-ministry-category
`poor-not-ministry-category`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ur neighborhoods. The Beatitude's "Blessed are the poor in spirit" (Matthew 5…

### Why Your Church Should Require a Sabbatical
`why-your-church-should-require-a-sabbatical`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …his work that he had done. So God blessed the seventh day and made it holy (…

### client/public/disruption/topics/consumer-comfort.json
`lib:client/public/disruption/topics/consumer-comfort.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …nished. More often the church has blessed the captivity instead of breaking…

### Why the Prophets Made Everyone Uncomfortable
`why-the-prophets-made-everyone-uncomfortable`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …onsciousness that wants everything blessed or the resentment that only wants…

### post-roe-church-responsibility
`post-roe-church-responsibility`

- _review_ · `foster` · 1× · review-word
  - literal 'foster care/parent/etc.' — almost certainly fine; flagged only because 'foster' is on the filler list
  - …ated the restrictions. Whether the foster care system that will bear some of…

### mission-sending-culture-never-sent
`mission-sending-culture-never-sent`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth shall be blessed — into his teaching, and returns…

### finding-your-barnabas
`finding-your-barnabas`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …e John Mark on a second missionary journey, John Mark having turned back from…

### Expository vs. Topical Preaching: Which and When
`expository-vs-topical-preaching`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …day of the church, and the Spirit blessed the breaking. Peter at Penteco…

### support-life-after-birth
`support-life-after-birth`

- _review_ · `foster` · 1× · review-word
  - literal 'foster care/parent/etc.' — almost certainly fine; flagged only because 'foster' is on the filler list
  - …ecific. It looks like adoption and foster care ministries that are as organi…

### client/public/disruption/topics/therapeutic-gospel.json
`lib:client/public/disruption/topics/therapeutic-gospel.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …hronicled its history in her study Blessed. Norman Vincent Peale and positiv…

### client/public/justice/topics/the-wage.json
`lib:client/public/justice/topics/the-wage.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …on a Monday. And the same church blessed the mill owner and ignored the mil…

### Why the Bible Is One Story, Not Sixty-Six Books
`why-the-bible-is-one-story-not-66-books`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …the families of the earth will be blessed (Genesis 12:3 ESV). The promise na…

### Fighting Fair: Conflict Without Casualties
`marriage-fighting-fair`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ace on the other side of it. Jesus blessed the second one and said nothing ab…

### client/public/context/guides/the-slavery-texts.json
`lib:client/public/context/guides/the-slavery-texts.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …serve as missionaries. Whips were blessed from pulpits. Frederick Douglass w…

### client/public/context/guides/money-texts-we-explain-away.json
`lib:client/public/context/guides/money-texts-we-explain-away.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …th work. The right spiritualizes. Blessed are the poor in spirit, the readin…

### client/public/leadership/articles/the-biblical-covenants.json
`lib:client/public/leadership/articles/the-biblical-covenants.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …atest son, in whom the nations are blessed at last. When you read of Abraham…

### client/public/leadership/articles/historical-context.json
`lib:client/public/leadership/articles/historical-context.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …ow slaveholders found a Bible that blessed slavery and how prosperity preache…

### client/public/leadership/articles/resilience-in-ministry.json
`lib:client/public/leadership/articles/resilience-in-ministry.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …Deep Roots Jeremiah describes the blessed man as a tree planted by water, se…

### client/public/leadership/articles/comparison-and-envy-in-ministry.json
`lib:client/public/leadership/articles/comparison-and-envy-in-ministry.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …es that God made a mistake when He blessed the other man. It resents the gift…

### client/public/leadership/articles/humility-in-leadership.json
`lib:client/public/leadership/articles/humility-in-leadership.json`

- _review_ · `blessed` · 1× · review-word
  - forbidden as an adjective ('blessed life'); fine as a verb ('God blessed them')
  - …y is self forgetfulness. It is the blessed relief of a man who has stopped ke…

### The Unfinished Church: Calling, Vision, and the Future God Is Building
`book:the-unfinished-church-calling-vision-and-the-future-god-is-building`

- _review_ · `journey` · 1× · review-word
  - forbidden only as a metaphor — fine if literal travel
  - …work. Bell reflects on the churchs journey, current challenges, and Gods prom…
