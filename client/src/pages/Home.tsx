...
export default function Home() {
  ...
  return (
    <div style={{ background: "#f8f8f6" }}>
      ...
      {/* HERO SECTION */}
      <section style={{ background: "#ffffff", color: "#111111", padding: "80px 20px", display: "flex", alignItems: "center", gap: "80px", maxWidth: "1400px", margin: "0 auto", minHeight: "600px", flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: "280px" }}>
          ...
          <p style={{ fontSize: "17px", lineHeight: "1.8", marginBottom: "32px", color: "#666666" }}>
          ...
          <div style={{ display: "flex", gap: "12px", marginBottom: "40px", flexWrap: "wrap" }}>
            <Link href="/start" style={{ textDecoration: "none" }}>
              <button style={{ background: "#D4A017", color: "#1A1A1A", border: "none", padding: "16px 32px", fontSize: "16px", fontWeight: "bold", borderRadius: "4px", cursor: "pointer" }}>
                Find Your Path
              </button>
            </Link>
            <Link href="/writing" style={{ textDecoration: "none" }}>
              <button style={{ background: "transparent", color: "#111111", border: "1px solid #e5e5e5", padding: "16px 32px", fontSize: "16px", borderRadius: "4px", cursor: "pointer" }}>
                Browse All Essays
              </button>
            </Link>
          </div>
          <div style={{ display: "flex", gap: "40px", fontSize: "14px", color: "#666666", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: "24px", flexWrap: "wrap" }}>
            <div><div style={{ fontSize: "24px", fontWeight: "bold", color: "#D4A017" }}>880+</div><div>Essays</div></div>
            <div><div style={{ fontSize: "24px", fontWeight: "bold", color: "#D4A017" }}>25</div><div>Books</div></div>
            <div><div style={{ fontSize: "24px", fontWeight: "bold", color: "#D4A017" }}>4,200+</div><div>Readers</div></div>
            <div><div style={{ fontSize: "24px", fontWeight: "bold", color: "#D4A017" }}>Free</div><div>to Start</div></div>
          </div>
        </div>
      ...
      {/* "START HERE" QUIZ CTA */}
      <section style={{ background: "#111111", color: "#ffffff", padding: "60px 20px", textAlign: "center" }}>
        <div style={{ maxWidth: "700px", margin: "0 auto" }}>
          <div style={{ fontSize: "12px", fontWeight: "bold", letterSpacing: "2px", color: "#D4A017", marginBottom: "12px" }}>NOT SURE WHERE TO START?</div>
          <h2 style={{ fontSize: "32px", fontWeight: "bold", marginBottom: "16px", fontFamily: "Georgia, serif" }}>Take the 2-Minute Path Quiz</h2>
          <p style={{ fontSize: "16px", color: "#666666", marginBottom: "32px", lineHeight: "1.7" }}>Answer three questions. Get a personalized reading path based on where you are and what you're carrying.</p>
          <Link href="/start" style={{ textDecoration: "none" }}>
      ...
      {/* FEATURED ESSAYS */}
      <section style={{ background: "#f8f8f6", color: "#1C1510", padding: "80px 20px" }}>
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: "24px", marginBottom: "40px", borderBottom: "1px solid #e5e5e5", paddingBottom: "16px", overflowX: "auto" }}>
            {['All', 'Marriage', 'Parenting', 'Theology', 'Justice', 'Pastoral'].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab.toLowerCase())} style={{ padding: "8px 0", background: "none", border: "none", borderBottom: activeTab === tab.toLowerCase() ? "3px solid #D4A017" : "none", color: activeTab === tab.toLowerCase() ? "#D4A017" : "#1C1510", fontWeight: activeTab === tab.toLowerCase() ? "bold" : "normal", fontSize: "14px", cursor: "pointer", whiteSpace: "nowrap" }}>{tab}</button>
            ))}
          </div>
          {filteredArticles.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "#6B7280" }}>
              <p style={{ fontSize: "18px", marginBottom: "16px" }}>Essays loading...</p>
              <Link href="/writing" style={{ textDecoration: "none", color: "#D4A017", fontWeight: "bold" }}>Browse all essays</Link>
            </div>
          ) : (
              ...
              {filteredArticles.map((article: any, i: number) => (
                <Link key={i} href={"/writing/" + article.slug} style={{ textDecoration: "none" }}>
                  <div style={{ background: "#FFF", padding: "24px", borderRadius: "8px", border: "1px solid #e5e5e5", cursor: "pointer", height: "100%", display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: "11px", fontWeight: "bold", color: "#D4A017", marginBottom: "8px", textTransform: "uppercase" }}>{article.topic || 'ESSAY'}</div>
          ...
          <div style={{ textAlign: "center", marginTop: "48px" }}>
            <Link href="/writing" style={{ textDecoration: "none" }}>
              <button style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "12px 32px", background: "#1A1A1A", color: "#ffffff", border: "none", borderRadius: "4px", fontWeight: "bold", fontSize: "14px", cursor: "pointer" }}>Browse All 880+ Essays</button>
            </Link>
          </div>
        </div>
      </section>
      ...
      {/* TESTIMONIALS */}
      <section style={{ background: "#111111", color: "#ffffff", padding: "80px 20px" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "16px", textAlign: "center", color: "#666666", marginBottom: "60px" }}>From pastors in crisis to couples on the edge to seekers with hard questions.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "24px" }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.05)", padding: "28px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div style={{ display: "flex", gap: "4px", marginBottom: "16px" }}>
                  {[...Array(t.rating)].map((_, j) => (<Star key={j} size={14} fill="#D4A017" color="#D4A017" />))}
                </div>
                <p style={{ fontSize: "14px", lineHeight: "1.8", marginBottom: "16px", fontStyle: "italic" }}>"{t.quote}"</p>
                <div style={{ fontSize: "12px" }}>
                  <div style={{ fontWeight: "bold" }}>{t.name}</div>
                  <div style={{ color: "#666666" }}>{t.role}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* NEWSLETTER SIGNUP */}
      <section style={{ background: "#111111", color: "#ffffff", padding: "80px 20px", textAlign: "center" }}>
        <div style={{ maxWidth: "600px", margin: "0 auto" }}>
          <div style={{ fontSize: "12px", fontWeight: "bold", letterSpacing: "2px", color: "#D4A017", marginBottom: "12px" }}>WEEKLY NEWSLETTER</div>
          <h2 style={{ fontSize: "32px", fontWeight: "bold", marginBottom: "16px", fontFamily: "Georgia, serif" }}>Theology for your week</h2>
          <p style={{ fontSize: "15px", color: "#666666", marginBottom: "32px", lineHeight: "1.7" }}>One essay a week. Marriage, parenting, faith, justice — whatever you're carrying. Free. No spam. Unsubscribe anytime.</p>
          {emailSubmitted ? (
            <p style={{ color: "#D4A017", fontWeight: "bold", fontSize: "18px" }}>You're in. Check your inbox.</p>
          ) : (
            <form onSubmit={handleEmailSubmit} style={{ display: "flex", gap: "12px", maxWidth: "480px", margin: "0 auto", flexWrap: "wrap", justifyContent: "center" }}>
              ...
              <button type="submit" style={{ background: "#D4A017", color: "#1A1A1A", border: "none", padding: "14px 28px", fontSize: "15px", fontWeight: "bold", borderRadius: "4px", cursor: "pointer", whiteSpace: "nowrap" }}>Subscribe Free</button>
            </form>
          )}
        </div>
      </section>
      ...
      <section style={{ background: "#f8f8f6", padding: "80px 20px" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", gap: "60px", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: "280px" }}>
            <div style={{ fontSize: "12px", fontWeight: "bold", letterSpacing: "2px", color: "#D4A017", marginBottom: "12px" }}>ABOUT JAMES BELL</div>
            ...
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "24px" }}>
              {["Lead Pastor", "Author of 25 Books", "PCN Founder", "Father of Five"].map((badge, i) => (
                <div key={i} style={{ background: "#111111", color: "#ffffff", padding: "6px 14px", borderRadius: "4px", fontSize: "12px", fontWeight: "600" }}>{badge}</div>
              ))}
            </div>
            <Link href="/about" style={{ textDecoration: "none" }}>
              <button style={{ background: "#D4A017", color: "#1A1A1A", border: "none", padding: "12px 24px", fontSize: "14px", fontWeight: "bold", borderRadius: "4px", cursor: "pointer" }}>Full Story</button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#111111", color: "#ffffff", padding: "60px 20px", textAlign: "center" }}>
        <div style={{ maxWidth: "700px", margin: "0 auto" }}>
          <div style={{ fontSize: "12px", fontWeight: "bold", letterSpacing: "2px", color: "#D4A017", marginBottom: "12px" }}>PASTORS CONNECTION NETWORK</div>
          ...
          <p style={{ fontSize: "15px", color: "#666666", marginBottom: "32px", lineHeight: "1.7" }}>43% of pastors report loneliness and isolation. You don't have to lead alone. Sermon resources, pastoral health articles, coaching, and a community of pastors who get it.</p>
          <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/pastors" style={{ textDecoration: "none" }}>
              <button style={{ background: "#D4A017", color: "#1A1A1A", border: "none", padding: "14px 28px", fontSize: "15px", fontWeight: "bold", borderRadius: "4px", cursor: "pointer" }}>Explore Pastor Resources</button>
            </Link>
            <a href="https://pastorsconnectionnetwork.com" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
              <button style={{ background: "transparent", color: "#111111", border: "1px solid #e5e5e5", padding: "14px 28px", fontSize: "15px", borderRadius: "4px", cursor: "pointer" }}>Visit PCN</button>
            </a>
          </div>
        </div>
      </section>
