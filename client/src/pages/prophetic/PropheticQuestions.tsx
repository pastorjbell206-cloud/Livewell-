/**
 * Shared hard-questions hub for a prophetic section. The questions a skeptical
 * reader arrives with, answered fairly from both directions and routed into the
 * topics. Loaded from <base>/questions.json.
 */
...
export default function PropheticQuestions({ config }: { config: SectionConfig }) {
  ...
  const topicReady = (slug: string) => config.topics.find((t) => t.slug === slug && t.ready);
    ...
    <Layout>
      ...
      <section style={{ background: "var(--charcoal)", padding: "var(--s-6) var(--s-4) var(--s-5)", color: "var(--bone)" }}>
        <div style={wrap}>
          ...
          <h1 style={{ fontFamily: "var(--F)", fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 400, lineHeight: 1.05, letterSpacing: "-0.025em", marginBottom: "16px", maxWidth: "20ch" }}>The suspicions, taken seriously.</h1>
          <p style={{ fontFamily: "var(--B)", fontSize: "18px", lineHeight: 1.7, color: "rgba(245,240,230,0.78)", maxWidth: "62ch" }}>
            Some hear 'justice' and brace for a political ambush. Others brace for a sermon that dodges the real thing. Both worries deserve an honest answer. Here they are.
          </p>
        </div>
      </section>
                  ...
                  <div style={{ padding: "0 var(--s-4) var(--s-4)", borderTop: "1px solid var(--border)" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center", marginTop: "16px", paddingTop: "12px", borderTop: "1px solid var(--border)" }}>
                      {q.topic && topicReady(q.topic) && (
                        <Link href={`${config.base}/topic/${q.topic}`} style={{ fontFamily: "var(--U)", fontSize: "13px", fontWeight: 600, color: "var(--charcoal)", background: "var(--mustard)", padding: "8px 14px", borderRadius: "999px" }}>
                          Go deeper: {topicReady(q.topic)!.title} →
                        </Link>
                      )}
                      {q.passages?.map((ref) => (
                        <Link key={ref} href={`/theology/passage?ref=${encodeURIComponent(ref)}`} style={{ fontFamily: "var(--U)", fontSize: "13px", fontWeight: 600, color: "var(--mustard-text)", border: "1px solid var(--mustard)", padding: "8px 12px", borderRadius: "999px" }}>{ref}</Link>
                      ))}
                    </div>
                  </div>
                )}
