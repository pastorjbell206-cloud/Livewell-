...
export function NotificationsAdmin() {
  ...
  const createNotification = trpc.notifications.create.useMutation({
    onSuccess: () => {
      addToast({ type: 'success', title: 'Notification created' });
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {notifications && notifications.length > 0 ? (
          notifications.map((notification: any) => (
            <div key={notification.id} className="bg-white border border-gray-200 rounded-lg p-4 flex items-start justify-between hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleDelete(notification.id)} className="p-2 hover:bg-red-50 rounded transition-colors" title="Delete">
          ...
          <div className="text-center py-8 text-gray-500"><p>No notifications yet</p></div>
        )}
      </div>
    </div>
    </AdminLayout>
