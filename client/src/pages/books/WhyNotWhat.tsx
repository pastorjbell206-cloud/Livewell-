/**
 * Why Not What (/why-not-what). The ebook sales page: cover, pitch, a free
 * sample (the opening, served from /books/why-not-what-sample.md), and a Buy
...
const SLUG = "why-not-what";
const TITLE = "Why Not What";
const SUBTITLE = "How Theology Starts With the Right Question";
const prose = { maxWidth: "var(--w-prose)", margin: "0 auto" } as const;

export default function WhyNotWhat() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/why-not-what-sample.md", { cache: "no-store" })
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            We have made Christianity into a vast answer to the question what should I do. The steps, the habits, the
            five points by Friday. And we are baffled when people do all of it and feel nothing, when they keep every
            rule and lose every reason. The trouble is not the what. It is the order. A faith built on what before why
            functions for a season and then fails exactly when you need it, because it was never standing on anything.
            This short book recovers the older order the whole Bible insists on. The catechism that began with why. The
            eleven chapters of Romans before the first command. The grace that came before the law at Sinai. Why first,
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            Once the why is real, the what takes care of itself. Not what. Why.
          </p>
