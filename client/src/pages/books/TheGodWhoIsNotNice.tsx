/**
 * The God Who Is Not Nice (/the-god-who-is-not-nice). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/the-god-who-is-not-nice-sample.md), and a Buy button. The
...
const SLUG = "the-god-who-is-not-nice";
const TITLE = "The God Who Is Not Nice";
const SUBTITLE = "Recovering the Weight of God in a Sentimental Age";
const prose = { maxWidth: "var(--w-prose)", margin: "0 auto" } as const;

export default function TheGodWhoIsNotNice() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/the-god-who-is-not-nice-sample.md", { cache: "no-store" })
    ...
    <Layout>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-7) var(--s-4) var(--s-6)" }}>
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          <img
            src="/books/the-god-who-is-not-nice.svg"
            alt={`${TITLE} cover`}
            width={210}
            height={315}
          ...
          <div style={{ flex: "1 1 320px" }}>
            <div style={{ fontFamily: "var(--U)", fontSize: "0.75rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "var(--mustard)" }}>
              New Ebook · On the Character of God
            </div>
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            We traded the holy, weighty, dangerous God of the Bible for one who exists to affirm us and keep us comfortable. A God who is nice. But nice is not holy, and a God with no weight cannot save, cannot comfort, and is not worth worshiping. This book recovers the God of Scripture in his full weight: his holiness, his otherness, his wrath as an aspect of his love, his beauty, his refusal to be managed. Not a mean God. A holy one, whose severity and tenderness are the same fire.
          </p>
          <p style={{ fontFamily: "var(--B)", fontSize: "16px", lineHeight: 1.7, color: "var(--ink-muted)" }}>
            By James Bell. A recovery of reverence for readers tired of a God too small to matter. Read the opening below, free.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            Nice is not holy. A God with no weight cannot carry yours.
          </p>
