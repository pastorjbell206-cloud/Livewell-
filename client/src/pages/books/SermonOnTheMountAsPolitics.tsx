/**
 * The Sermon on the Mount as Politics (/sermon-on-the-mount-as-politics). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/sermon-on-the-mount-as-politics-sample.md), and a Buy button. The
...
const SLUG = "sermon-on-the-mount-as-politics";
...
export default function SermonOnTheMountAsPolitics() {
    ...
    <Layout>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-7) var(--s-4) var(--s-6)" }}>
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          <img
            src="/books/sermon-on-the-mount-as-politics.svg"
            alt={`${TITLE} cover`}
            width={210}
            height={315}
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            The meek will inherit the earth. Not the strong. We were never meant to keep this sermon on the page.
          </p>
