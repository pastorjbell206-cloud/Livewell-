/**
 * How to Read the Bible (/how-to-read-the-bible). The ebook sales page: cover,
 * pitch, a free sample (the opening, served from
 * /books/how-to-read-the-bible-sample.md), and a Buy button. The full book is
...
const SLUG = "how-to-read-the-bible";
const TITLE = "How to Read the Bible";
const SUBTITLE = "Without Making It Say What You Already Believe";
...
export default function HowToReadTheBible() {
  ...
  useEffect(() => {
    fetch("/books/how-to-read-the-bible-sample.md", { cache: "no-store" })
        ...
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 320px" }}>
            <div style={{ fontFamily: "var(--U)", fontSize: "0.75rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "var(--mustard)" }}>
              New Ebook · On Reading Scripture
            </div>
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            Everyone reads the Bible through a lens, and most of us call it just reading. We mine it for verses that
            arm the position we already held. We put one line on a mug and skip the paragraph around it. The skeptic
            says the Bible means whatever you want, and the way we handle it keeps proving him right. This short book
            teaches the older, harder skill: reading that submits to the text instead of conscripting it, from
            proof-texting to genre to the passages we would rather skip.
          </p>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            You came holding the text. It ends holding you.
          </p>
