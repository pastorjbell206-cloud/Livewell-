/**
 * Marriage in Ministry (/marriage-in-ministry). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/marriage-in-ministry-sample.md), and a Buy button. The
...
const SLUG = "marriage-in-ministry";
...
export default function MarriageInMinistry() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/marriage-in-ministry-sample.md", { cache: "no-store" })
    ...
    <Layout>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-7) var(--s-4) var(--s-6)" }}>
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          <img
            src="/books/marriage-in-ministry.svg"
            alt={`${TITLE} cover`}
            width={210}
            height={315}
          ...
          <div style={{ flex: "1 1 320px" }}>
            <div style={{ fontFamily: "var(--U)", fontSize: "0.75rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "var(--mustard)" }}>
              New Ebook · On Marriage & Ministry
            </div>
