/**
 * The Body You Left (/the-body-you-left). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/the-body-you-left-sample.md), and a Buy button. The
...
const SLUG = "the-body-you-left";
const TITLE = "The Body You Left";
const SUBTITLE = "A Case for the Church in an Age That Walked Away";
const prose = { maxWidth: "var(--w-prose)", margin: "0 auto" } as const;

export default function TheBodyYouLeft() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/the-body-you-left-sample.md", { cache: "no-store" })
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            People are leaving the church in enormous numbers, and many of them are right about what drove them out. This book takes the dechurched seriously. It names honestly what the church did to lose them, the abuse, the hypocrisy, the politics, the shallowness, and then it makes the hard case for why the church still matters, why the faith was never meant to be practiced alone, why the body of Christ is not optional even when it is disappointing. Not a guilt trip to fill the pews. An honest reckoning, and a costly case.
          </p>
          <p style={{ fontFamily: "var(--B)", fontSize: "16px", lineHeight: 1.7, color: "var(--ink-muted)" }}>
            By James Bell, a pastor writing from inside the church's failures, not above them. Read the opening below, free.
          </p>
        </div>
      </section>
