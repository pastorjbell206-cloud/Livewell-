/**
 * Alone in a Crowded Church (/alone-in-a-crowded-church). The ebook sales page:
 * cover, pitch, a free sample (the opening, served from
 * /books/alone-in-a-crowded-church-sample.md), and a Buy button. The full book
...
const SLUG = "alone-in-a-crowded-church";
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            He is in a room full of people who love him, and he has never felt more alone. That is the quiet
            condition of a great many pastors, and almost none of them will say it out loud. This short book names
            the lie that made the loneliness feel holy, walks through what Scripture and church history actually
            say about leading in brotherhood instead of isolation, and ends on the one step that begins to undo it.
          </p>
          <p style={{ fontFamily: "var(--B)", fontSize: "16px", lineHeight: 1.7, color: "var(--ink-muted)" }}>
            By James Bell, lead pastor and founder of the Pastors Connection Network. Read the opening below, free.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: "var(--charcoal)", color: "var(--charcoal-fg)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            The rest of the book is waiting, and so is the brother you have not called yet.
          </p>
