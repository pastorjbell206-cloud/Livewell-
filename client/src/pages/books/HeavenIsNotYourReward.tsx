/**
 * Heaven Is Not Your Reward (/heaven-is-not-your-reward). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/heaven-is-not-your-reward-sample.md), and a Buy button. The
...
const SLUG = "heaven-is-not-your-reward";
const TITLE = "Heaven Is Not Your Reward";
const SUBTITLE = "The Resurrection Hope the Church Traded for an Escape";
const prose = { maxWidth: "var(--w-prose)", margin: "0 auto" } as const;

export default function HeavenIsNotYourReward() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/heaven-is-not-your-reward-sample.md", { cache: "no-store" })
    ...
    <Layout>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-7) var(--s-4) var(--s-6)" }}>
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          <img
            src="/books/heaven-is-not-your-reward.svg"
            alt={`${TITLE} cover`}
            width={210}
            height={315}
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            The Bible does not promise you an escape from the world. It promises the world made new, and you raised in it.
          </p>
