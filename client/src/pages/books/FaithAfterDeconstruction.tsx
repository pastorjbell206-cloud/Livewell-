/**
 * Faith After Deconstruction (/faith-after-deconstruction). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/faith-after-deconstruction-sample.md), and a Buy button. The
...
const SLUG = "faith-after-deconstruction";
...
export default function FaithAfterDeconstruction() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/faith-after-deconstruction-sample.md", { cache: "no-store" })
    ...
    <Layout>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-7) var(--s-4) var(--s-6)" }}>
        <div style={{ ...prose, display: "flex", gap: "44px", alignItems: "center", flexWrap: "wrap" }}>
          <img
            src="/books/faith-after-deconstruction.svg"
            alt={`${TITLE} cover`}
            width={210}
            height={315}
          ...
          <div style={{ flex: "1 1 320px" }}>
            <div style={{ fontFamily: "var(--U)", fontSize: "0.75rem", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.18em", color: "var(--mustard)" }}>
              New Ebook · For the Deconstructing
            </div>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            Some things in your faith should fall. The cornerstone is not one of them.
          </p>
