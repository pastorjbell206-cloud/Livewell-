/**
 * HealWell (/healwell). The ebook sales page: cover, pitch, a free sample
 * (the opening, served from /books/healwell-sample.md), and a Buy button. The
...
const SLUG = "healwell";
const TITLE = "HealWell";
const SUBTITLE = "52 Weeks in Costly Hope";
const prose = { maxWidth: "var(--w-prose)", margin: "0 auto" } as const;

export default function HealWell() {
  const [sample, setSample] = useState<string | null>(null);

  useEffect(() => {
    fetch("/books/healwell-sample.md", { cache: "no-store" })
      ...
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4) var(--s-4)" }}>
        <div style={prose}>
          <p style={{ fontFamily: "var(--B)", fontSize: "19px", lineHeight: 1.8, color: "var(--ink)" }}>
            A devotional that will not lie to you. Fifty-two weeks of honest readings for tired believers and for people in pain, written from inside the wound and not from a safe distance above it. It does not tie a bow on suffering. It names the grief and the doubt and the silence truthfully, and then it points, without denial, to a hope that costs something and holds.
          </p>
          <p style={{ fontFamily: "var(--B)", fontSize: "16px", lineHeight: 1.7, color: "var(--ink-muted)" }}>
            By James Bell. A full year of costly hope, one week at a time. Read the opening below, free.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bone)", padding: "var(--s-6) var(--s-4)" }}>
        <div style={{ ...prose, textAlign: "center" }}>
          <p style={{ fontFamily: "var(--F)", fontSize: "24px", fontStyle: "italic", margin: "0 0 22px", color: "rgba(245,240,230,.92)" }}>
            He heals the brokenhearted. He does not pretend they were never broken.
          </p>
