/**
 * Post-purchase download page for Is Critical Race Theory Biblical? Stripe's
 * Payment Link is configured to redirect here after a successful payment. Links
 * the EPUB + PDF directly.
 */
import Layout from "@/components/Layout";
import { SEOMeta } from "@/components/SEOMeta";
import { Link } from "wouter";

const DIR = "/ebook/critical-race-theory-biblical";
const EPUB = `${DIR}/Is-Critical-Race-Theory-Biblical.epub`;
const PDF = `${DIR}/Is-Critical-Race-Theory-Biblical.pdf`;
...
export default function IsCriticalRaceTheoryBiblicalThankYou() {
  return (
    <Layout>
      <SEOMeta title="Thank you — your download" description="Download your copy of Is Critical Race Theory Biblical?" noindex />
      <section style={{ background: "var(--bone)", padding: "var(--s-6) var(--s-4)", minHeight: "60vh" }}>
        <div style={{ maxWidth: "var(--w-prose)", margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "var(--B)", fontSize: "18px", lineHeight: 1.7, color: "var(--ink-muted)", maxWidth: "54ch", margin: "0 auto 36px" }}>
            Thank you for buying <em>Is Critical Race Theory Biblical?</em> Download both formats below — the EPUB for your e-reader or phone, the PDF for everything else. The links stay on this page, so bookmark it.
          </p>
          ...
          <p style={{ fontFamily: "var(--B)", fontSize: "14px", color: "var(--ink-muted)", marginTop: "40px" }}>
            ...
            and we'll send your files directly. <Link href="/books/critical-race-theory-biblical" style={{ color: "var(--ink)", borderBottom: "1px solid var(--mustard)" }}>Back to the book</Link>.
          </p>
        </div>
      </section>
    </Layout>
