 ...
 * their sub-pathways. Single source of truth for the primary nav dropdowns,
 * the admin sub-pathway select, and the filtered listing pages.
 *
...
/** Flat list of every sub-pathway, tagged with its parent pillar. */
export const ALL_SUBPATHWAYS: { pillar: string; label: string; slug: string }[] =
  PILLAR_ORDER.flatMap((pillar) =>
    PILLAR_SUBPATHWAYS[pillar].map((s) => ({ pillar, label: s.label, slug: s.slug }))
  );

/** sub-pathway slug → label (slugs are globally unique). */
export const SUBPATHWAY_LABEL_BY_SLUG: Record<string, string> = Object.fromEntries(
  ALL_SUBPATHWAYS.map((s) => [s.slug, s.label])
);

/** sub-pathway label → its parent pillar name. */
export const PILLAR_BY_SUBPATHWAY_LABEL: Record<string, string> = Object.fromEntries(
  ALL_SUBPATHWAYS.map((s) => [s.label, s.pillar])
);
