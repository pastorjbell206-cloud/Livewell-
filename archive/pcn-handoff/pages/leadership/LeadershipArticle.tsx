...
export default function LeadershipArticle() {
  ...
  const [result, setResult] = useState<{ slug: string; data: Article | null } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/leadership/articles-index.json")
      ...
      .then((idx: { articles: IndexEntry[] } | null) => {
        ...
        const sameGroup = others.filter((a) => a.group === group);
        const rest = others.filter((a) => a.group !== group);
